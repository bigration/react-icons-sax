// Auto-generated file
import * as React from 'react';
import { SVGProps } from 'react';
type IconProps = {
  size?: number;
};
const SvgGrid8 = (props: IconProps & SVGProps<SVGSVGElement>) => {
  const mergedStyles = {
    display: 'inline-block',
    fill: undefined,
    height: props.size || 24,
    width: props.size || 24,
  };
  return (
    <svg
      width="1em"
      height="1em"
      viewBox={props.viewBox || '0 0 24 24'}
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      color={props.color || undefined}
      style={mergedStyles}
      {...props}
    >
      <path
        opacity={0.4}
        d="M22 7.81V12h-6.25V2h.44C19.83 2 22 4.17 22 7.81Z"
        fill="currentColor"
      />
      <path
        d="M22 12v4.19c0 3.64-2.17 5.81-5.81 5.81h-.44V12H22ZM8.25 12v10h-.44C4.17 22 2 19.83 2 16.19V12h6.25Z"
        fill="currentColor"
      />
      <path
        opacity={0.4}
        d="M7.81 2C4.17 2 2 4.17 2 7.81V12h6.25V2h-.44Z"
        fill="currentColor"
      />
      <path d="M15.75 2h-7.5v10h7.5V2Z" fill="currentColor" />
      <path opacity={0.4} d="M15.75 12h-7.5v10h7.5V12Z" fill="currentColor" />
    </svg>
  );
};
export default SvgGrid8;
